...
- [X] 5b.0 **User Story:** As a VPN user, I want VPN instance
  containers to route all received traffic through their
  tunnel so that domain-resolved public IPs exit via the
  company VPN, not via the VPS internet [4/4]
  - [X] 5b.1 In `l2tp/entrypoint.sh` `setup_routing()`: after
...
- [X] 6.0 **User Story:** As a VPN user, I want to deploy and
  ...
  - [X] 6.2 Run `python3 generate-vpn.py` and verify
    ...
    [verify: manual-run-user]
  - [X] 6.3 Check route-init logs for domain resolution and
    /32 route creation:
    `./rdocker.sh compose logs route-init | grep domain`
    [verify: manual-run-user]
    → domain routing loop running, TTL-aware polling [live] (2026-04-11)
  - [X] 6.4 From Mac via Tailscale exit node, verify traffic
    to the configured domain goes through the VPN instance
    (not Mullvad): `curl` or `traceroute` to the domain
    [verify: manual-run-user]
    → confirmed by user, no failures [live] (2026-04-11)

- [X] 7.0 **User Story:** As a VPN user, I want the PRD and
  tech design marked complete so that the feature is properly
  closed out [2/2]
  - [X] 7.1 Update PRD status from Draft to Complete
    [verify: code-only]
  - [X] 7.2 Update tech design status from Draft to Complete
    [verify: code-only]
